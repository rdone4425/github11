...
confirm_install() {
    ...
    echo "• 程序文件 (/opt/file-sync-system)"
    echo "• 配置文件模板"
    echo "• 完整文档"
    echo ""
...
show_post_install() {
    ...
    echo ""
    echo "1. 配置GitHub凭据："
    echo -e "   ${YELLOW}nano /opt/file-sync-system/config/global.conf${NC}"
    echo "   • 设置 GITHUB_USERNAME"
    echo "   • 设置 GITHUB_TOKEN (Personal Access Token)"
    echo ""
    echo "2. 配置监控路径："
    echo -e "   ${YELLOW}nano /opt/file-sync-system/config/paths.conf${NC}"
    echo "   • 设置要监控的本地目录"
    echo "   • 设置对应的GitHub仓库"
    echo ""
    echo "3. 验证配置："
    ...
    echo ""
    echo -e "${BLUE}📚 文档位置：${NC}"
    echo "• 安装指南: /opt/file-sync-system/docs/installation.md"
    echo "• 配置说明: /opt/file-sync-system/docs/configuration.md"
    echo "• 使用说明: /opt/file-sync-system/docs/usage.md"
    echo ""
